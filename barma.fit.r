...
barma.fit<- function (y, ar, ma, link, names_phi,names_theta,names_beta,diag,h1,X,X_hat,resid)
{
  ...
  { 
    ...
    escore <- function(z) 
    {
      ...
      mT <- diag(mu.eta(eta[(m+1):n])) #mu changed by eta[(m+1):n]
            #mT <- diag(mu.eta(mu))
	...
	mT <- diag(mu.eta(etahat[(m+1):n])) # CHANGED muhat by etahat[(m+1):n]
    #N TROQUEI
  ...
  if(any(is.na(ar)==F) && any(is.na(ma)==T) && any(is.na(X)==T))
  {
    ...
    escore <- function(z) 
    {
      ...
      mT <- diag(mu.eta(eta[(m+1):n])) #changed
      
      Ualpha <- prec * sum((mu.eta(mu)) * (ystar-mustar)) #WHY sum? ##mu por eta[(m+1):n] sera?
    ...
    psi1 = trigamma(muhat * prec)
    psi2 = trigamma((1 - muhat) * prec)
    mT <- diag(mu.eta(etahat[(m+1):n])) #muhat por etahat[(m+1):n]
    W = diag(c(prec * (psi1 + psi2))) %*% mT^2
    vc = prec * (psi1 * muhat - psi2 * (1 - muhat))
    D = diag(c(psi1 * (muhat^2) + psi2 * (1 - muhat)^2 - trigamma(prec)))
  ...
  { 
    ...
    escore <- function(z) 
    {
      ...
      mT <- diag(mu.eta(eta[(m+1):n])) #mu changed by aquilo
  ...
  { 
    ...
    escore <- function(z) 
    {
      ...
      mT <- diag(mu.eta(eta[(m+1):n])) #changed mu by eta[(m+1):n]
  ...
  # adicionar quantilico em RESID 3 e comentar acima
  ...
  z$resid2 <- sign(y[(m+1):n]-z$fitted[(m+1):n])*sqrt(2*(dt)) #deviance
  
   # WRONG z$resid3 <- as.vector(qnorm(pbeta(y[(m+1):n],z$fitted[(m+1):n],z$prec))) #quantilico
  z$resid3 <- qnorm(pbeta(y[(m+1):n], z$fitted[(m+1):n]*z$prec, (1-z$fitted[(m+1):n])*z$prec))
  ...
  z$resid4 <- (errorhat[(m+1):n])/sqrt(muhat*(1-muhat)/(1+prec))
...
#     plot(residc,main=" ",xlab="�?ndices",ylab="res�?duos", pch = "+",ylim=c(-4,4))
...
#     if(diag>1)
#     {
#       pdf(file = "resid_v_ind.eps",width = w1, height = h1,family = "Times")
#       {
#         par(mfrow=c(1,1))
...
#       dev.off()
#       
#       pdf(file = "obs_v_fit.eps",width = w1, height = h1,family = "Times")
#       {
#         par(mfrow=c(1,1))
#         par(mar=c(2.8, 2.7, 1, 1)) # margens c(baixo,esq,cima,direia)
#         par(mgp=c(1.7, 0.45, 0))
...
#       dev.off()
#       
#       pdf(file = "resid_density.eps",width = w1, height = h1,family = "Times")
#       {
#         par(mfrow=c(1,1))
...
#       dev.off()
#       
#       pdf(file = "resid_FAC.eps",width = w1, height = h1,family = "Times")
#       {
#         par(mfrow=c(1,1))
#         par(mar=c(2.8, 2.7, 1, 1)) # margens c(baixo,esq,cima,direia)
#         par(mgp=c(1.7, 0.45, 0))
...
#       dev.off()
#       
#       pdf(file = "resid_FACP.eps",width = w1, height = h1,family = "Times")
#       {
#         par(mfrow=c(1,1))
#         par(mar=c(2.8, 2.7, 1, 1)) # margens c(baixo,esq,cima,direia)
#         par(mgp=c(1.7, 0.45, 0))
...
#       dev.off()
#       
#       pdf(file = "qq_plot.eps",width = w1, height = h1,family = "Times")
#       {  
#         par(mfrow=c(1,1))
#         par(mar=c(2.8, 2.7, 1, 1)) # margens c(baixo,esq,cima,direia)
#         par(mgp=c(1.7, 0.45, 0))
...
#       dev.off()
#       
#       pdf(file = "adjusted.eps",width = 6, height = 4,family = "Times")
#       {
#         par(mfrow=c(1,1))
#         par(mar=c(2.8, 2.7, 1, 1)) # margens c(baixo,esq,cima,direia)
#         par(mgp=c(1.7, 0.45, 0))
...
#       dev.off()
#       
#       pdf(file = "forecast.eps",width = 6, height = 4,family = "Times")
#       {
#         par(mfrow=c(1,1))
#         par(mar=c(2.8, 2.7, 1, 1)) # margens c(baixo,esq,cima,direia)
#         par(mgp=c(1.7, 0.45, 0))
