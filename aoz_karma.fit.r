...
karma.fit.aoz<- function (y, ar, ma, a, b, link, names_phi,names_theta,names_beta,diag=0,h1,X,X_hat,resid=3, lambda=5)
{
  maxit1<-1000
  z <- c()
  ...
  linkfun <- link$linkfun  # (mu)
  linkinv <- link$linkinv  # (eta)
  mu.eta  <-  link$mu.eta  # dmu/deta
  diflink <- link$diflink  # function(t) 1/(stats$mu.eta(stats$linkfun(t)))
  ...
  if(any(is.na(ar)==F) && any(is.na(ma)==F) && any(is.na(X)==T))
  {
    ...
    rP <- deta.dphi[(m+1):n,]
    rR <- deta.dtheta[(m+1):n,]
    rL <- matrix(as.vector(dmu.dlambda[(m+1):n]),ncol=1) # include

    mT <- diag(mu.eta(etahat[(m+1):n],lambda))
    ...
    Kal <- t(rL) %*% W %*% mT %*% v # alpha lambda
    Kla <- t(Kal)
    Kpl <- t(rP) %*% W %*% mT %*% rL # phi lambda
    Klp <- t(Kpl)
    Ktl <- t(rR) %*% W %*% mT %*% rL # theta lambda
    Klt <- t(Ktl)
    Kprecl <- sum(D%*%rL) # prec lambda
    Klprec <- t(Kprecl)
    Kll <- sum(W%*%rL^2) # lambda lambda
    # Kbl <- t(rM) %*% W %*% mT %*% rL #  beta lambda
    # Klb <- t(Kbl)
  ...
  if(any(is.na(ar)==F) && any(is.na(ma)==F) && any(is.na(X)==F))
  {
    ...
    reg <- c(mqo[1:(p1+1)], rep(0,q1), prec, lambda, beta1) # initializing the parameter values
    ...
    opt.i <- optim(reg, loglik, hessian = T,
                   # escore,
                   method = "SANN",
                   control = list(fnscale = -1))#, maxit = maxit1, reltol = 1e-12))
    reg.i <- (opt.i$par)[1:(p1+q1+3+ncol(X))]

    opt <- optim(reg.i, loglik, hessian = T,
                 escore,
                 method = "BFGS",
                 control = list(fnscale = -1))#, maxit = maxit1, reltol = 1e-12))
    ...
    colnames(K) <- names_par
    rownames(K) <- names_par
...
#     EQM <- mean((res1[(m+1):n])^2); print(c(">> EQM in-sample:", EQM), quote = F)
#     MAPE<- mean(abs((y[(m+1):n]-z$fitted[(m+1):n])/y[(m+1):n])); print(c(">> MAPE in-sample:", MAPE), quote = F)
#     dif.y <- mean(abs(diff(y[(m+1):n])))
#     MASE= mean(abs((y[(m+1):n]-z$fitted[(m+1):n]))/dif.y); print(c(">> MASE in-sample:", MASE), quote = F)
...
#     par(mfrow=c(1,1))
#     plot(y,type="l",ylab="serie",xlab="tempo")
#     lines(z$fitted, lty=5)
#     legend("bottom",c("Valores observados","Valores estimados"), #pch=c(1,5),
#            pt.bg="white", lty=c(1,5), bty="n", cex=.85)
# 
# 
# 
#     w1<-5
