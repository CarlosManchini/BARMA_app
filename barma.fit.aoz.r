...
barma.fit.aoz<- function (y, ar, ma, link, names_phi,names_theta,names_beta,diag,h1,X,X_hat,resid,lambda=1.5)
{
  maxit1<-503# apos o lzera reduzi de 1000 para 500
  z<-c()
  ...
  { 
    ...
    if(any(is.na(X)==F))
    {
      ...
      z$hq <- -2*(z$loglik*(n/(n-m)))+log(log(n))*(p1+q1+3+length(beta))
    }else{
      z$aic <- -2*z$loglik+2*(p1+q1+2)
      z$bic <- -2*z$loglik+log(n)*(p1+q1+2)
      z$hq <- -2*(z$loglik*(n/(n-m)))+log(log(n))*(p1+q1+3)
    }
  } 
  ...
  #     legend("topright",c("Densidade estimada","Normal padr?o"),#pch=vpch,
  ...
  # 
  #     par(mfrow=c(1,1))
  #     plot(y,type="l",ylab="Umidade relativa do ar",xlab="Tempo")
  #     lines(z$fitted,type = "l",lty=2)
  #     legend(2007.1,.69, lty = c(1,2), cex=0.96 ,bty ="n", 
  #            legend = c("Dados observados","Valores ajustados"))
  #     
  ...
  #         lines(densidade$x,dnorm(densidade$x),lty=2)
  #         legend("topleft",c("Densidade estimada","Normal padrao"),#pch=vpch,
  #                pt.bg="white", lty=c(1,2), bty="n")
  #       }
  #       dev.off()
  # 
